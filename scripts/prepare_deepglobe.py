...
if __name__ == "__main__":
    ...
    ap.add_argument("--root", required=True, help="Racine DeepGlobe")
    ap.add_argument("--out", required=True, help="Dossier de sortie tiles")
    ap.add_argument("--size", type=int, default=512)
    ap.add_argument("--overlap", type=int, default=64)
    args = ap.parse_args()

    # DeepGlobe multi-classes (adapter au schéma local)
    class_map = {"building": 1, "road": 2}
    run(args.root, args.out, args.size, args.overlap, vector_key="class", class_map=class_map)
